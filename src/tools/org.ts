...
import { resolveCommandSubdomain, type CommandSubdomainSource } from "../services/command-subdomain.js";

export function registerOrgTools(server: McpServer): void {
  server.registerTool(
    "verkada_get_org_info",
    {
  ...
  - Use when: "Check MCP config before building a history footage URL"`,
    ...
    async () => {
      try {
        ...
        let commandSubdomain: string | undefined = configured;
        let source: CommandSubdomainSource | undefined = configured ? "config" : undefined;
