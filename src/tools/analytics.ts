...
Examples:
  ...
  - Use when: "Who changed door settings today?"`,
    ...
    async ({ start_time, end_time, page_size, page_token }) => {
      try {
        ...
        const data = await client.get<unknown>("/core/v1/audit_log", { start_time, end_time, page_size, page_token });
  ...
  // -----------------------------------------------------------------------
  server.registerTool(
    "verkada_list_license_plates_of_interest",
    {
...
Examples:
  ...
  - Use when: "List vehicles of interest"`,
    ...
    async () => {
      try {
        const client = getRequestClient();
        const data = await client.get<unknown>("/cameras/v1/analytics/lpr/license_plate_of_interest");
        return { content: [{ type: "text" as const, text: truncate(JSON.stringify(data, null, 2), CHARACTER_LIMIT) }] };
      } catch (err) {
        return { content: [{ type: "text" as const, text: `Error: ${formatError(err)}` }], isError: true };
      }
  ...
  // -----------------------------------------------------------------------
  server.registerTool(
    "verkada_create_license_plate_of_interest",
    {
...
Examples:
  - Use when: "Flag license plate ABC123 as suspicious"`,
    ...
    async ({ license_plate, description }) => {
      try {
        const client = getRequestClient();
        const data = await client.post<unknown>("/cameras/v1/analytics/lpr/license_plate_of_interest", {
          license_plate,
          description,
        });
  ...
  // -----------------------------------------------------------------------
  server.registerTool(
    "verkada_delete_license_plate_of_interest",
    {
...
Examples:
  - Use when: "Remove license plate ABC123 from the flagged list"`,
    ...
    async ({ license_plate }) => {
      try {
        const client = getRequestClient();
        const data = await client.delete<unknown>("/cameras/v1/analytics/lpr/license_plate_of_interest", {
          license_plate,
        });
...
Examples:
  ...
  - Use when: "Get LPR detections with images from the parking lot camera"`,
    ...
    async ({ camera_id, license_plate, start_time, end_time, page_size, page_token }) => {
      try {
        const client = getRequestClient();
        const data = await client.get<unknown>("/cameras/v1/analytics/lpr/images", {
          camera_id,
          license_plate,
          start_time,
          end_time,
...
Examples:
  ...
  - Use when: "Track visits by vehicle XYZ789 at the entrance camera"`,
    ...
    async ({ camera_id, license_plate, start_time, end_time, page_size, page_token }) => {
      try {
        const client = getRequestClient();
        const data = await client.get<unknown>("/cameras/v1/analytics/lpr/timestamps", {
          camera_id,
          license_plate,
          start_time,
          end_time,
  ...
  // -----------------------------------------------------------------------
  server.registerTool(
    "verkada_list_persons_of_interest",
    {
...
Examples:
  ...
  - Use when: "List all flagged individuals"`,
    ...
    async () => {
      try {
        const client = getRequestClient();
        const data = await client.get<unknown>("/cameras/v1/people/person_of_interest");
        return { content: [{ type: "text" as const, text: truncate(JSON.stringify(data, null, 2), CHARACTER_LIMIT) }] };
      } catch (err) {
        return { content: [{ type: "text" as const, text: `Error: ${formatError(err)}` }], isError: true };
      }
  ...
  // -----------------------------------------------------------------------
  server.registerTool(
    "verkada_create_person_of_interest",
    {
...
Examples:
  - Use when: "Add this person as a POI with label 'Shoplifter'"`,
    ...
    async ({ label, face_image_base64 }) => {
      try {
        const client = getRequestClient();
        const data = await client.post<unknown>("/cameras/v1/people/person_of_interest", {
          label,
  ...
  // -----------------------------------------------------------------------
  server.registerTool(
    "verkada_delete_person_of_interest",
    {
...
Examples:
  - Use when: "Remove person of interest ID abc123"`,
    ...
    async ({ person_id }) => {
      try {
        const client = getRequestClient();
        const data = await client.delete<unknown>("/cameras/v1/people/person_of_interest", { person_id });
        return { content: [{ type: "text" as const, text: JSON.stringify(data ?? { success: true }, null, 2) }] };
      } catch (err) {
        return { content: [{ type: "text" as const, text: `Error: ${formatError(err)}` }], isError: true };
      }
...
Examples:
  ...
  - Use when: "Get air quality data for the last week"`,
    ...
    async ({ device_id, start_time, end_time }) => {
      try {
        const client = getRequestClient();
        const data = await client.get<unknown>("/environment/v1/data", {
          device_id,
          start_time,
          end_time,
        });
