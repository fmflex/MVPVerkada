...
export function registerAccessTools(server) {
    // -----------------------------------------------------------------------
    // List Access Users
    // -----------------------------------------------------------------------
    server.registerTool("verkada_list_access_users", {
...
Examples:
  ...
  - Use when: "Find user by email or name"`,
        ...
        try {
            ...
            const data = await client.get("/access/v1/access_users", { include_visitors });
    ...
    // Get Access User
    // -----------------------------------------------------------------------
    server.registerTool("verkada_get_access_user", {
...
Examples:
  ...
  - Use when: "What credentials does user_id abc123 have?"`,
        ...
        }).strict().refine((d) => d.user_id || d.external_id || d.email || d.employee_id, "Provide at least one of: user_id, external_id, email, or employee_id"),
        annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    }, async ({ user_id, external_id, email, employee_id }) => {
        try {
            const client = getRequestClient();
            const data = await client.get("/access/v1/access_users/user", {
                user_id,
                external_id,
                email,
                employee_id,
    ...
    // -----------------------------------------------------------------------
    server.registerTool("verkada_unlock_door_admin", {
...
WARNING: This is a privileged action that bypasses all access controls.`,
        ...
        try {
            const client = getRequestClient();
            const data = await client.post("/access/v1/door/admin_unlock", { door_id });
            return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
        }
        catch (err) {
            return { content: [{ type: "text", text: `Error: ${formatError(err)}` }], isError: true };
    ...
    // Unlock Door as User
    // -----------------------------------------------------------------------
    server.registerTool("verkada_unlock_door_user", {
...
Examples:
  - Use when: "Unlock door for user john (user_id: abc)"`,
        ...
        }).strict().refine((d) => Boolean(d.user_id) !== Boolean(d.external_id), "Provide exactly one of: user_id or external_id (not both)"),
        annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: false },
    }, async ({ door_id, user_id, external_id }) => {
        try {
            const client = getRequestClient();
            const data = await client.post("/access/v1/door/user_unlock", {
                door_id,
    ...
    // Get Doors
    // -----------------------------------------------------------------------
    server.registerTool("verkada_get_doors", {
        title: "Get Doors",
...
Examples:
  ...
  - Use when: "Get doors in site abc123"`,
        ...
        }).strict().refine((d) => !(d.site_ids && d.door_ids), "Provide only one of site_ids or door_ids, not both"),
        annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
        ...
        try {
            ...
            const params = {};
            if (site_ids)
                params.site_ids = site_ids.join(",");
            if (door_ids)
                params.door_ids = door_ids.join(",");
            const data = await client.get("/access/v1/doors", params);
            return { content: [{ type: "text", text: truncate(JSON.stringify(data, null, 2), CHARACTER_LIMIT) }] };
        }
        catch (err) {
            return { content: [{ type: "text", text: `Error: ${formatError(err)}` }], isError: true };
    ...
    // Get Access Groups
    // -----------------------------------------------------------------------
    server.registerTool("verkada_list_access_groups", {
...
Examples:
  ...
  - Use when: "List all security groups"`,
        ...
        try {
            const client = getRequestClient();
            const data = await client.get("/access/v1/access_groups");
            return { content: [{ type: "text", text: truncate(JSON.stringify(data, null, 2), CHARACTER_LIMIT) }] };
        }
        catch (err) {
            return { content: [{ type: "text", text: `Error: ${formatError(err)}` }], isError: true };
    ...
    // -----------------------------------------------------------------------
    server.registerTool("verkada_add_user_to_group", {
...
Examples:
  - Use when: "Add user john to the Contractors group"`,
        ...
        }).strict().refine((d) => d.user_id || d.external_id, "Provide at least one of: user_id or external_id"),
        annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false },
    }, async ({ group_id, user_id, external_id }) => {
        try {
            const client = getRequestClient();
            const data = await client.put("/access/v1/access_groups/group/user", user_id ? { user_id } : { external_id }, { group_id });
            return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
        }
        catch (err) {
            return { content: [{ type: "text", text: `Error: ${formatError(err)}` }], isError: true };
    ...
    // -----------------------------------------------------------------------
    server.registerTool("verkada_remove_user_from_group", {
...
Examples:
  - Use when: "Remove user from the Visitors group"`,
        ...
        try {
            ...
            const data = await client.delete("/access/v1/access_groups/group/user", params);
            return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
        }
        catch (err) {
            return { content: [{ type: "text", text: `Error: ${formatError(err)}` }], isError: true };
    ...
    // Get Access Events
    // -----------------------------------------------------------------------
    server.registerTool("verkada_get_access_events", {
...
Examples:
  ...
  - Use when: "Show all access events in the last hour"`,
        ...
        try {
            const client = getRequestClient();
            const data = await client.get("/events/v1/access", {
                start_time,
                end_time,
                device_id,
                site_id,
    ...
    // -----------------------------------------------------------------------
    server.registerTool("verkada_add_access_card", {
...
Examples:
  - Use when: "Add HID card 1234 with facility code 56 to user abc"`,
        ...
        }).strict().refine((d) => d.user_id || d.external_id, "Provide at least one of: user_id or external_id").refine((d) => (d.facility_code !== undefined && d.card_number !== undefined) || d.card_number_hex, "Provide facility_code + card_number OR card_number_hex"),
        annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: false },
    }, async ({ user_id, external_id, type, facility_code, card_number, card_number_hex, active }) => {
        try {
            const client = getRequestClient();
            const data = await client.post("/access/v1/credentials/card", { type, facility_code, card_number, card_number_hex, active }, { user_id, external_id });
            return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
        }
        catch (err) {
            return { content: [{ type: "text", text: `Error: ${formatError(err)}` }], isError: true };
    ...
    // -----------------------------------------------------------------------
    server.registerTool("verkada_set_user_pin", {
...
Examples:
  - Use when: "Set PIN 1234 for user abc"`,
        ...
        try {
            const client = getRequestClient();
            const data = await client.put("/access/v1/access_users/user/entry_code", { entry_code: pin }, { user_id, external_id });
            return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
        }
        catch (err) {
            return { content: [{ type: "text", text: `Error: ${formatError(err)}` }], isError: true };
