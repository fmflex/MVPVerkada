...
export function registerUserTools(server) {
    ...
    // Create User
    // -----------------------------------------------------------------------
    server.registerTool("verkada_create_user", {
...
Examples:
  - Use when: "Create a new user for employee John Doe"`,
        ...
        }).strict().refine((d) => d.email || d.first_name || d.last_name, "Provide at least one of: email, first_name, or last_name"),
        annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: false },
    }, async ({ external_id, email, first_name, last_name }) => {
        try {
            const client = getRequestClient();
            const data = await client.post("/core/v1/user", {
                external_id,
                email,
                first_name,
                last_name,
    ...
    // Get User
    // -----------------------------------------------------------------------
    server.registerTool("verkada_get_user", {
        title: "Get Org User",
...
Examples:
  - Use when: "Get user details for external ID emp123"`,
        ...
        try {
            const client = getRequestClient();
            const data = await client.get("/core/v1/user", { user_id, external_id });
            return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
        }
        catch (err) {
            return { content: [{ type: "text", text: `Error: ${formatError(err)}` }], isError: true };
    ...
    // Update User
    // -----------------------------------------------------------------------
    server.registerTool("verkada_update_user", {
...
Examples:
  - Use when: "Update email for user emp123 to new@example.com"`,
        ...
        try {
            const client = getRequestClient();
            const data = await client.put("/core/v1/user", { email, first_name, last_name }, { user_id, external_id });
            return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
        }
        catch (err) {
            return { content: [{ type: "text", text: `Error: ${formatError(err)}` }], isError: true };
    ...
    // Delete User
    // -----------------------------------------------------------------------
    server.registerTool("verkada_delete_user", {
...
WARNING: This permanently removes the user.`,
        ...
        try {
            const client = getRequestClient();
            const data = await client.delete("/core/v1/user", { user_id, external_id });
