...
export function buildCameraHistoryUrl(cameraId, startTimeSeconds, durationSeconds, orgSubdomain) {
    const timestampMs = startTimeSeconds * 1000;
    return (`https://${orgSubdomain}.command.verkada.com/cameras/${cameraId}` +
...
export function adjustFootageUrlDuration(url, durationSeconds, startTimeSeconds) {
    const timestampMs = startTimeSeconds * 1000;
    try {
...
export function registerCameraTools(server) {
    // -----------------------------------------------------------------------
    // Get Camera Data
    // -----------------------------------------------------------------------
    server.registerTool("verkada_get_cameras", {
...
Examples:
  ...
  - Use when: "Find camera by serial number"`,
        ...
        try {
            ...
            const data = await client.get("/cameras/v1/devices");
    ...
    // Get Alerts
    // -----------------------------------------------------------------------
    server.registerTool("verkada_get_alerts", {
  ...
  - Use when: "List license plate of interest alerts this week"`,
        inputSchema: z.object({
            ...
            notification_type: z
                .string()
                .optional()
                .describe('Comma-separated notification types (e.g. "motion,tamper"). Valid: person_of_interest, license_plate_of_interest, tamper, crowd, motion, camera_offline, camera_online, line_crossing, loitering'),
        }).strict(),
        annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    }, async ({ start_time, end_time, include_image_url, page_size, page_token, notification_type }) => {
        try {
            const client = getRequestClient();
            const data = await client.get("/cameras/v1/alerts", {
                start_time,
                end_time,
                include_image_url,
                page_size,
    ...
    // -----------------------------------------------------------------------
    server.registerTool("verkada_get_object_counts", {
...
Examples:
  ...
  - Use when: "Show vehicle counts for the parking camera last week"`,
        ...
        try {
            const client = getRequestClient();
            const data = await client.get("/cameras/v1/analytics/object_counts", {
                camera_id,
                start_time,
                end_time,
                page_size,
    ...
    // -----------------------------------------------------------------------
    server.registerTool("verkada_get_latest_thumbnail", {
...
Examples:
  ...
  - Use when: "What does the entrance camera look like right now?"`,
    ...
    // -----------------------------------------------------------------------
    server.registerTool("verkada_get_camera_history_url", {
  ...
  - Use when: "Share a 10-minute clip starting at this alert timestamp"`,
        ...
        try {
            ...
            if (subdomain) {
                ...
                const data = await client.get("/cameras/v1/footage/link", {
                    camera_id,
                    timestamp: start_time,
                });
                if (!data.url) {
                    throw new Error("Footage link API returned no url. Set VERKADA_COMMAND_SUBDOMAIN or pass org_subdomain to build the link locally.");
                }
            ...
            return {
                content: [{ type: "text", text: JSON.stringify({ url, note }, null, 2) }],
            };
        }
        catch (err) {
            return {
    ...
    // Get Streaming Token
    // -----------------------------------------------------------------------
    server.registerTool("verkada_get_streaming_token", {
...
Examples:
  - Use when: "I need to stream live footage from a camera"`,
        ...
        try {
            const client = getRequestClient();
            const data = await client.request("GET", "/cameras/v1/footage/token", { useApiKey: true });
            return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
        }
        catch (err) {
            return { content: [{ type: "text", text: `Error: ${formatError(err)}` }], isError: true };
